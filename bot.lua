...
-- Main function to decide the next action for the counter-strategy.
function decideCounterAction()
  local player = LatestGameState.Players[ao.id]

  -- Look for a potential target to exploit
  for target, state in pairs(LatestGameState.Players) do
    if target ~= ao.id and inRange(player.Position.x, player.Position.y, state.Position.x, state.Position.y, 3) then
      -- Engage in hit-and-run tactics if we have enough energy
      if player.energy > 10 and player.health > 30 then
        print("Engaging in hit-and-run tactic against player " .. target .. ".")
        ao.send({Target = Game, Action = "PlayerAttack", Player = ao.id, AttackEnergy = tostring(player.energy / 3), TargetPlayer = target})
  ...
  -- Default action: move to a safe position if possible
  local safeDirection = findSafeDirection(player.Position, LatestGameState.Players)
  if safeDirection then
    print("Moving to a safer position in direction: " .. safeDirection)
    ao.send({Target = Game, Action = "PlayerMove", Player = ao.id, Direction = safeDirection})
...
-- Handler to decide the next best action for the counter-strategy.
Handlers.add(
  "decideCounterAction",
  ...
  function (msg)
    if not InAction then
      ...
      elseif playerEnergy == 0 then
        ...
        ao.send({Target = Game, Action = "PlayerAttack", Player = ao.id, AttackEnergy = tostring(playerEnergy / 3), TargetPlayer = attackerId})
      end
      InAction = false
      ao.send({Target = ao.id, Action = "Tick"})
    else
